...
async function confirmBulkDelete() {
  ...
  toast.success(`${ids.length} activity log entr${ids.length === 1 ? 'y' : 'ies'} deleted.`)
  await loadPage(currentPage.value)
}
  ...
  <AppLayout>
    <div class="p-6 sm:p-8 space-y-6">
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr v-if="!loading && !logs.length">
                <td colspan="6" class="py-10 text-center text-faint">No activity recorded yet.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <ConfirmDialog
        ...
        :title="`Delete ${selectedIds.length} activity log entr${selectedIds.length === 1 ? 'y' : 'ies'}?`"
