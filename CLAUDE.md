...
- `composables/useTableSort.js` (click-to-sort, dot-path support for nested fields e.g. `category.name`) and `useTableFilter.js` (chainable exact-match dropdown filters) are applied across all 15 SPA list pages, composed with the existing `useTableSearch.js` and a shared `TableSortIcon` component / `.th-sort` / `.filter-select` CSS classes. Add new list pages using this trio rather than hand-rolling sort/filter. The one exception is **Activity Logs**, which is server-paginated — its sort only applies within the currently loaded page, not the full dataset.
