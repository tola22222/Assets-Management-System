...
<div class="max-w-2xl mx-auto px-4 sm:px-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8">
        <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit Category: {{ $category->name }}</h1>
        <form action="{{ route('categories.update', $category) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-1">Name *</label>
                <input type="text" name="name" value="{{ $category->name }}" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand focus:border-brand outline-none">
            </div>
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-1">Short Name</label>
                <input type="text" name="short_name" list="category-codes" maxlength="6" value="{{ old('short_name', $category->short_name) }}"
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand focus:border-brand outline-none">{{ $category->description }}</textarea>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <button type="submit" class="w-full sm:w-auto px-6 py-2.5 bg-brand text-white rounded-xl text-sm font-bold hover:bg-brand-dark text-center">Update Category</button>
