...
<template>
  <Modal :title="title" @close="$emit('close')">
    <!-- The body owns the card gutter now, so the rows only carry their own
         vertical rhythm; the divider then stops at the gutter like a list rule. -->
    <div class="modal-body">
      <dl class="divide-y divide-line">
        <div
          v-for="(row, i) in rows"
          :key="i"
          class="grid grid-cols-1 sm:grid-cols-[150px_minmax(0,1fr)] gap-x-6 gap-y-1 py-3.5"
        >
