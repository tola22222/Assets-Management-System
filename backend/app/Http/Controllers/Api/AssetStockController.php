...
class AssetStockController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $assets = DB::transaction(function () use ($validated, $referenceNo) {
            $created = [];
        ...
        return response()->json([
            'reference_no' => $referenceNo,
            'assets' => collect($assets)->map->load(['category', 'location']),
        ], 201);
    }
    ...
    public function destroy(AssetMovement $asset_stock)
    {
        ...
        return response()->json(['message' => 'Receipt record removed. The asset itself was not deleted.']);
    }
}
