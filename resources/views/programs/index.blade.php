            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="text-gray-400 dark:text-gray-500 font-semibold bg-gray-50/70 dark:bg-gray-800/70 border-b border-gray-100 dark:border-gray-700">
                        <th class="p-4 pl-5 font-semibold tracking-wide">Program Name</th>
                        <th class="p-4 font-semibold tracking-wide hidden md:table-cell">Description</th>
                        <th class="p-4 font-semibold tracking-wide hidden md:table-cell">Created Date</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 font-normal text-gray-600 dark:text-gray-400">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition">
                        <td class="p-4 pl-5 font-medium text-gray-900 dark:text-white">{{ $program->name }}</td>
                        <td class="p-4 hidden md:table-cell text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ $program->description ?? 'No description provided' }}</td>
                        <td class="p-4 hidden md:table-cell text-gray-500 dark:text-gray-400">{{ $program->created_at?->format('M d, Y') ?? 'No Date' }}</td>
                        <td class="p-4 pr-5">
                            <div class="flex items-center justify-center gap-1.5">
                                <button onclick="openModal('edit', {{ json_encode($program) }})"
                                ...
                                <form action="{{ route('programs.destroy', $program->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this program?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <p class="text-gray-400 dark:text-gray-500 text-sm font-medium">No programs found yet.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<div id="programModal" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm hidden z-[100] flex items-center justify-end p-4">
    <div class="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden animate__slide-in-right">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800">
            <h3 id="modalTitle" class="text-lg font-bold text-gray-900 dark:text-white tracking-wide">Add New Program</h3>
        ...
        <form id="programForm" method="POST" class="flex-1 overflow-y-auto">
            @csrf
            <div id="methodField"></div>
            <div class="p-6 space-y-5 bg-gray-50/30 dark:bg-gray-900/30">
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 dark:text-gray-300 tracking-wide">Program Name <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="prog_name" placeholder="e.g. ICT, English, Dream" required
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" id="prog_desc" rows="4" placeholder="Brief details about this program..."
...
<script>
    ...
    function openModal(mode, data = null) {
        ...
        if (mode === 'edit') {
            ...
            form.action = `/programs/${data.id}`;
            methodField.innerHTML = '@method("PUT")';
            document.getElementById('prog_name').value = data.name;
            document.getElementById('prog_desc').value = data.description || '';
        } else {
            modalTitle.innerText = 'Add New Program';
