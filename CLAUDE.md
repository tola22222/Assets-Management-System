...
**Repo layout**: the Laravel app was restructured out of the repo root into `backend/` (commit `e9a61ff`, "Restructure repo into backend/frontend split; add Vuetify architecture") — `backend/` and `frontend/` are now sibling directories, with `Dockerfile`, `docker-compose.yml`, `nginx/`, and `.github/workflows/` at the repo root referencing both. **Run all `composer`/`artisan` commands from `backend/`, not the repo root.**
...
- `npm run build` and `npm run build:local` (mode `artisan`) — both build to `frontend/dist/` with base `/app/` (a `/` base previously caused a prod white-screen: the built HTML requested assets at the domain root instead of `/app/assets/*`). **Do not reintroduce a `/` base for the default build.** In production this is copied to `backend/public/app/` (see `Dockerfile`) and served by nginx — both the default host (via Laravel's `spa` route) and `app.pepyasset.online` reach it at `/app/`.
- **Known gap**: `backend/routes/web.php`'s `spa` route serves `/app` locally by looking for the build at `base_path('frontend/dist')`, i.e. inside the Laravel root, falling back to `public_path('app')`. Post-restructure `frontend/` is a sibling of `backend/`, not nested inside it, so that first path never resolves anymore — the "single-server, no-Vite" local flow the route's own comments describe is currently broken (it silently falls through to `public/app`, which is empty locally). For local SPA development, use the two-process flow (`npm run dev` + `php artisan serve`) instead.

### Local single-server serving (Blade side)
`backend/routes/web.php` has a `spa` route (`/app/{path?}`) that serves the built SPA (see the known gap above), and `/` redirects to `/app` (keeping the `dashboard` route name). The legacy Blade dashboard lives at `/blade`.

Note: there are **two `package.json` / Vite setups**. `backend/package.json` builds Blade assets (`backend/resources/`, Tailwind CSS v4 + `laravel-vite-plugin`); `frontend/package.json` builds the Vue SPA (Vuetify, no Tailwind). They are independent.
...
- **UI kit is Vuetify** (`vite-plugin-vuetify`, autoImport), configured in `frontend/src/plugins/vuetify.js` with two custom themes (`pepyLight`/`pepyDark`) carrying the PEPY brand palette. **Tailwind has been retired from the SPA** — `frontend/src/main.css` now only sets Vuetify's `--v-font-body`/`--v-font-heading` variables to `Kantumruy Pro`. Use Vuetify components/props (`v-btn`, `density`, `variant`, etc.) in new SPA components, not Tailwind utility classes; a handful of not-yet-migrated components may still carry dead Tailwind class names left over from the old design system, so don't treat their presence as the current convention.
- `frontend/src/composables/useThemeColor.js` — the Settings "Theme Color" picker converts a chosen hex to an HSL-derived shade scale and mutates the live Vuetify theme's `primary` color at runtime (no rebuild needed), persisted to `localStorage` and re-applied on boot via a side-effect import in `main.js`.
...
- `pages/<module>/Index.vue` — one folder per domain module, mostly CRUD index pages driven by the `useApiCrud.js` composable (plus `useServerTable.js`, `useColumnVisibility.js`, `useConfirm.js` for shared table/dialog behavior).
...
- `layouts/` — `AppLayout` (authed shell, plus `AppHeader`/`AppNavigationDrawer`) vs `AuthLayout` (login).
...
- Table row actions (Edit/Delete/Approve/Reject/etc.) across CRUD pages use a `v-btn icon="mdi-dots-vertical"` menu trigger opening a dropdown of Vuetify menu items, rather than separate icon buttons — copy the pattern from `pages/assets/Index.vue` rather than inventing a new one.
...
## Deployment
Pushing to `main` triggers `.github/workflows/deploy.yml`: builds the multi-stage `Dockerfile` (Vue build from `frontend/` → PHP-FPM Alpine image built from `backend/`, with the SPA baked into `backend/public/app/`), pushes to Docker Hub, then SSHes into an EC2 host and `docker compose up`s app + nginx + MySQL. The deploy script writes `docker-compose.yml` and `nginx/default.conf` inline on the server, waits for MySQL, then runs `migrate --force` and re-caches config/route/view.
...
The container `docker/entrypoint.sh` fixes storage perms, waits for the DB, and runs a background loop calling `php artisan schedule:run` every 60s (there is no system cron).
