...
<div id="addModal" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl animate-in fade-in zoom-in duration-200">
        ...
        <form action="{{ route('suppliers.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" class="w-full mt-1 p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="e.g. Apple Store" required>
            </div>
            <div>
                <label class="text-xs font-bold text-slate-500 uppercase">Phone Number</label>
                <input type="text" name="phone" class="w-full mt-1 p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="012 345 678">
            </div>
            <div>
                <label class="text-xs font-bold text-slate-500 uppercase">Address</label>
                <textarea name="address" rows="3" class="w-full mt-1 p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Phnom Penh, Cambodia"></textarea>
            </div>
            <div class="flex gap-2 justify-end pt-2">
                <button type="button" onclick="document.getElementById('addModal').classList.add('hidden')" class="px-4 py-2 text-slate-500 font-medium">Cancel</button>
                <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 transition">Save Supplier</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl animate-in fade-in zoom-in duration-200">
        ...
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div class="flex gap-2 justify-end pt-2">
                <button type="button" onclick="closeEditModal()" class="px-4 py-2 text-slate-500 font-medium">Cancel</button>
                <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 transition">Update Supplier</button>
            </div>
        </form>
    </div>
</div>
