...
const { t } = useI18n()
const { items: movements, loading, fetchAll, destroy, destroyMany } = useApiCrud('/asset-movements', { entityName: t('asset_movements.title') })
...
const { selectedIds, allSelected, toggleSelectAll, toggleSelect, clearSelection } = useBulkSelect(sortedMovements)
...
<template>
  <AppLayout>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="m in sortedMovements" :key="m.id">
                <td>
                  <input type="checkbox" :checked="selectedIds.includes(m.id)" @change="toggleSelect(m.id)" class="rounded border-line text-brand focus:ring-brand/30" />
                </td>
                <td>
              ...
              <tr v-if="!loading && !sortedMovements.length">
                <td colspan="8" class="py-10 text-center text-faint">{{ t('asset_movements.empty') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <ConfirmDialog
      ...
      :title="`Delete ${selectedIds.length} movement${selectedIds.length === 1 ? '' : 's'}?`"
