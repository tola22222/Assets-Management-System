            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="text-gray-400 dark:text-gray-500 font-semibold bg-gray-50/70 dark:bg-gray-800/70 border-b border-gray-100 dark:border-gray-700">
                        <th class="p-4 pl-5 font-semibold tracking-wide">Supplier Name</th>
                        <th class="p-4 font-semibold tracking-wide hidden md:table-cell">Phone</th>
                        <th class="p-4 font-semibold tracking-wide hidden md:table-cell">Address</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 font-normal text-gray-600 dark:text-gray-400">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition">
                        <td class="p-4 pl-5 font-medium text-gray-900 dark:text-white">{{ $supplier->name }}</td>
                        <td class="p-4 hidden md:table-cell text-gray-500 dark:text-gray-400">{{ $supplier->phone ?? 'N/A' }}</td>
                        <td class="p-4 hidden md:table-cell text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $supplier->address ?? 'N/A' }}</td>
                        <td class="p-4 pr-5">
                            <div class="flex items-center justify-center gap-1.5">
                                <button onclick="openModal('edit', {{ json_encode($supplier) }})"
                                ...
                                <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this supplier?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <p class="text-gray-400 dark:text-gray-500 text-sm font-medium">No suppliers found yet.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<div id="supplierModal" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm hidden z-[100] flex items-center justify-end p-4">
    <div class="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden animate__slide-in-right">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800">
            <h3 id="modalTitle" class="text-lg font-bold text-gray-900 dark:text-white tracking-wide">Add New Supplier</h3>
        ...
        <form id="supplierForm" method="POST" class="flex-1 overflow-y-auto">
            @csrf
            ...
            <div class="p-6 space-y-5 bg-gray-50/30 dark:bg-gray-900/30">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="sup_name" placeholder="e.g. Tech Distributors Ltd." required
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 dark:text-gray-300 tracking-wide">Phone Number</label>
                    <input type="text" name="phone" id="sup_phone" placeholder="e.g. +855 12 345 678"
                        class="w-full bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-brand transition placeholder-gray-300 dark:placeholder-gray-500 dark:text-gray-200">
                </div>
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 dark:text-gray-300 tracking-wide">Address</label>
                    <textarea name="address" id="sup_address" rows="3" placeholder="Street, city, etc."
...
<script>
    ...
    function openModal(mode, data = null) {
        ...
        if (mode === 'edit') {
            ...
            form.action = `/suppliers/${data.id}`;
            methodField.innerHTML = '@method("PUT")';
            document.getElementById('sup_name').value = data.name;
