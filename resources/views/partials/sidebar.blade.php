<aside class="w-72 bg-white border-r border-slate-200 hidden lg:flex flex-col h-screen sticky top-0">
    ...
    <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('purchase-orders.index') }}" class="{{ $base }} {{ request()->routeIs('purchase-orders.*') ? $active : $inactive }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <a href="{{ route('categories.index') }}" class="{{ $base }} {{ request()->routeIs('categories.*') ? $active : $inactive }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <a href="{{ route('inventories.index') }}" class="{{ $base }} {{ request()->routeIs('inventories.*') ? $active : $inactive }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4a2 2 0 012-2m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
            </svg>
            Inventories
        </a>
    ...
    <div class="p-4 border-t border-slate-100">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-red-500 hover:bg-red-50 rounded-xl transition-colors duration-200 group">
