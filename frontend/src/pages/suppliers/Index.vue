...
const columns = computed(() => [
  ...
  { title: t('common.phone'), key: 'phone', render: (row) => row.phone || '—' },
  { title: t('common.address'), key: 'address', render: (row) => row.address || '—' },
  {
  ...
  <AppLayout>
    <div class="p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div class="table-wrap">
        ...
        >
          <template #empty>
            <p class="py-10 text-center text-faint text-sm">{{ search ? t('suppliers.empty_search') : t('suppliers.empty') }}</p>
          </template>
        </n-data-table>
      </div>
    </div>
