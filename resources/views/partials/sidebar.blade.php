@php
    ...
    $groups = [
        ...
        'settings'  => ['categories.*', 'assets-locations.*', 'suppliers.*', 'conditions.*'],
    ];
    ...
    <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('dashboard') }}"
            ...
            <div class="flex items-center gap-3">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2 7-7 7 7M5 10v10a1 1 0 001 1h3m10-11v10a1 1 0 01-1 1h-3" />
                </svg>
                Dashboard
            </div>
        </a>
        ...
        <div class="space-y-1">
            <button @click="openGroup = openGroup === 'inventory' ? '' : 'inventory'"
                ...
                <svg class="w-4 h-4 transition-transform duration-200" :class="openGroup === 'inventory' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-show="openGroup === 'inventory'" x-cloak x-collapse class="space-y-1">
                ...
                <a href="{{ route('asset-assignments.index') }}" class="{{ $subLinkClass }} {{ request()->routeIs('asset-assignments.*') ? $activeSubLink : $inactiveSubLink }}">Assignments</a>
                <a href="{{ route('asset-verifications.index') }}" class="{{ $subLinkClass }} {{ request()->routeIs('asset-verifications.*') ? $activeSubLink : $inactiveSubLink }}">Verification</a>
            </div>
        </div>
        ...
        <div class="space-y-1">
            <button @click="openGroup = openGroup === 'people' ? '' : 'people'"
                ...
                <svg class="w-4 h-4 transition-transform duration-200" :class="openGroup === 'people' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
        ...
        <div class="space-y-1">
            <button @click="openGroup = openGroup === 'settings' ? '' : 'settings'"
                ...
                <svg class="w-4 h-4 transition-transform duration-200" :class="openGroup === 'settings' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
